        ...
        for chameleon_weight in range(0, max_weight, weight_resulotion):
            for snake_weight in range(0, max_weight, weight_resulotion):
                for cheetah_weight in range(0, max_weight, weight_resulotion):
                    ...
                    data.append([wasp_weight, chameleon_weight, snake_weight, cheetah_weight, res[b_value], res[r_value], res[d_value], special_prob_dist[1]/sum(special_prob_dist), draw_prob_dist[1]/sum(draw_prob_dist)])
                    idx += 1
    df = pd.DataFrame(data, columns=['wasp', 'chameleon', 'snake', 'cheetah', 'black per', 'red per', 'draw per', 'special prob', 'draw prob'])
    return df
...
def plot_games():
    ...
    tooltips = [("wasp", "@wasp"), ("chameleon", "@chameleon"), ("snake", "@snake"), ("cheetah", "@cheetah"),("black red diff", "@black_red_diff")]
